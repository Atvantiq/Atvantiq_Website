  ...
  return (
    <div className="bg-gradient-to-br from-white to-gray-50 py-20 px-4 relative overflow-hidden">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Cards in straight V-shape */}
        <div className="relative w-full flex justify-center">
          <div className="flex flex-row items-start gap-3">
            {points.map((point, index) => {
              const mid = Math.floor(points.length / 2); // middle index
              // ↓ Middle card goes lowest, sides higher
              const flowOffset = (mid - Math.abs(index - mid)) * -60; 

              return (
                <div
                  key={index}
                  className="group relative w-[280px] p-3 bg-white/90 rounded-2xl shadow-md transition-all duration-300 hover:shadow-xl hover:-translate-y-3 hover:bg-[#2674D3]/5"
