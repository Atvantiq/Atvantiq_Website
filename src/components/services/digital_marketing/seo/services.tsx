...
import { Search, BarChart2, Layout, Wrench, BookOpen, MapPin, Link, Activity } from 'lucide-react';
        ...
        <div className="relative w-full min-h-[600px] flex items-center justify-center">
          <div className="relative grid grid-cols-1 lg:grid-cols-2 gap-8">
            {services.map((service, index) => (
              <div
                key={index}
                className="group relative w-full h-[150px] p-6 bg-white/95 rounded-xl shadow-lg border border-gray-100 transition-all duration-500 hover:shadow-xl hover:border-[#2674D3] hover:bg-gradient-to-br hover:from-white hover:to-gray-50"
                  ...
                  <div className="relative w-16 h-16 mr-6">
                    <div
                      className={`absolute inset-0 rounded-full bg-[#2674D3]/15 transition-all duration-300 ${
                        hoveredService === index ? 'scale-120 opacity-90 animate-pulse' : 'opacity-40'
                      }`}
                    ></div>
                    <div
                    ...
                    <ul className="text-sm text-gray-600 space-y-1 list-none">
                      {service.points.map((point, pointIndex) => (
                        <li key={pointIndex} className="flex items-center">
                          <span className="w-2 h-2 bg-[#2674D3] rounded-full mr-2"></span>
                          {point}
                        </li>
                      ))}
                    </ul>
