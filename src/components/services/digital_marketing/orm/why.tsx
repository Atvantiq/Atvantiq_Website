...
const ORM = () => {
  ...
  return (
    <div className="relative w-full py-20 bg-gradient-to-br from-white to-gray-50 md:h-screen">
      <div className="max-w-6xl mx-auto px-6">
        <h2 className="text-4xl font-bold text-center mb-16 md:mb-65 text-gray-800">
        ...
        <div className="flex flex-col gap-10 md:hidden">
          {points.map((point, index) => (
            ...
            >
              <div className="w-16 h-16 rounded-full flex items-center justify-center bg-white border-2 border-[#2674D3] shadow-md mb-4">
                {React.createElement(point.icon, {
                  className: "w-8 h-8 text-[#2674D3]",
                })}
              </div>
              <h3 className="text-lg font-semibold text-gray-800">{point.title}</h3>
              <p className="text-sm text-gray-600 mt-1 max-w-xs">{point.description}</p>
            </div>
          ))}
        </div>
      </div>
