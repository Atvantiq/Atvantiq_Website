...
// Moved data arrays outside the component to prevent re-creation on every render
...
const techCategories = [
  ...
  "AI & ML",
  "DevOps"
] as const; // Use 'as const' to improve TypeScript type inference
...
const IndustriesSection = () => {
  // Use 'as TechCategory' for better typing consistency
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-18 items-start">
          ...
          <div className="lg:col-span-1">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {industries.map((industry, index) => (
                <div
                  key={index}
                  // OPTIMIZATION: Added translate-z-0 for GPU acceleration on hover scale
                  className="flex flex-col items-center text-center group hover:transform hover:scale-105 transition-all duration-300 cursor-pointer transform translate-z-0"
                >
                  <Image
                    src={industry.icon}
                    alt={industry.name}
          ...
          <div className="lg:col-span-1 pt-4 md:pt-18">
            <div className="bg-gradient-to-br from-[#2674D3] to-[#2861B3] rounded-3xl p-8 relative overflow-hidden max-w-2xl mx-auto md:min-h-[510px]">
              ...
              <div className="flex gap-4 md:gap-6 h-full">
                {/* Categories Sidebar */}
                <div className="flex flex-col space-y-4 md:space-y-6 min-w-[140px]">
                  {techCategories.map((category) => (
                    <button
                      key={category}
                      onClick={() => setSelectedCategory(category as TechCategory)}
                      // OPTIMIZATION: Added translate-z-0 for GPU acceleration on transition
                      className={`text-left py-1 px-2 rounded-lg border border-white/20 transition-all duration-300 transform translate-z-0 ${
                ...
                <div className="flex-1">
                  {/* NOTE: No need for a separate transition component here. 
                         The technology cards update when selectedCategory changes, 
                         but the card animation is only on hover. */}
                  <div className="grid grid-cols-2 gap-2 md:gap-4">
                    {technologies[selectedCategory]?.slice(0, 8).map((tech, index) => (
                      <div
                        key={index}
                        // OPTIMIZATION: Added translate-z-0 for GPU acceleration on hover scale/shadow
                        className="bg-white rounded-xl p-2 flex flex-col items-center justify-center text-center transition-all duration-300 hover:scale-105 hover:shadow-lg transform translate-z-0"
                      >
                        <div 
                          className="text-[10px] md:text-2xl font-bold mb-2"
                          style={{ color: tech.color }}
                        >
                          {tech.logo}
                        </div>
                        <h4 className="text-[9px] md:text-sm font-semibold text-gray-800">
                          {tech.name}
                        </h4>
                      </div>
                    ))}
                    ...
                    {selectedCategory === "Frontend" && technologies[selectedCategory]?.length > 8 && (
                      <div className="col-span-2 flex justify-center">
                        <div 
                           // OPTIMIZATION: Added translate-z-0 for GPU acceleration on hover scale/shadow
                          className="bg-white rounded-xl p-2 flex flex-col items-center justify-center text-center w-[calc(50%-0.5rem)] transition-all duration-300 hover:scale-105 hover:shadow-lg transform translate-z-0"
                        >
                          <div 
